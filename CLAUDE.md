...
## CRITICAL: Manifest Updates When Creating/Modifying Plugins

**There are TWO manifest files that MUST be kept in sync:**

1. **`.claude-plugin/marketplace.json`** - Used by Claude Code marketplace to show available plugins
2. **`plugin-manifest.json`** - Detailed registry generated by build script
...
{
  "name": "aai-new-plugin",
  "source": "./plugins/aai-new-plugin",
  "description": "Brief description",
  "version": "1.0.0",
  "category": "stack|dev|pm|etc",
  "tags": ["relevant", "tags"]
}
```

2. **Bump the version** in `.claude-plugin/marketplace.json` `metadata.version` field

3. **Run build script** to update `plugin-manifest.json`:
```bash
...
1. Bump the plugin's version in both manifests
2. Bump the marketplace version in `.claude-plugin/marketplace.json`
3. Run `node scripts/build-manifest.js`
...
- [ ] **NEW PLUGIN: Added to `.claude-plugin/marketplace.json`**
- [ ] **Bumped version in `.claude-plugin/marketplace.json` metadata**
- [ ] **Ran `node scripts/build-manifest.js`**
...
8. **ALWAYS update both manifests** - Add new plugins to `.claude-plugin/marketplace.json` AND run `node scripts/build-manifest.js`
9. **ALWAYS bump marketplace version** - Increment `metadata.version` in `.claude-plugin/marketplace.json` when adding/modifying plugins
